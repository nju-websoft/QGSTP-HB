...
public class GenerateSubDBPedia extends ProcessBase {
    ...
    protected GenerateSubDBPedia(String graphname) {
        super(graphname);
    }
    String subdbname;
    ...
    void bfs() throws Exception {
        ...
        final int MAX_OUT=size/5000;
        final int MAX_HOP=20;
        while(!q.isEmpty()){
            int t=q.poll();
            if(d[t]>=MAX_HOP){
                continue;
    ...
    void generate(int size) throws Exception {
        this.size=size;
        ...
        stmt.executeUpdate("create table nodes engine="+ENGINE +
                " select A.id id,B.name name,B.weight weight from subnodemap A,"+dbname+".nodes B where A.origin=B.id");
        stmt.executeUpdate("create table edges engine="+ENGINE +
                " select A.id x,C.id y from subnodemap A,"+dbname+".edges B,subnodemap C where A.origin=B.x and B.y=C.origin");
//        stmt.executeUpdate("create table labels_list engine="+ENGINE +
//                " select A.id id,B.label label from subnodemap A,"+dbname+".labels_list B where A.origin=B.id");
        if("Jaccard".equals(Config.SD)) {
            stmt.executeUpdate("create table nodetype engine=" + ENGINE +
                    " select A.id node,B.type type from subnodemap A," + dbname + ".nodetype B where A.origin=B.node");
        }else{
            stmt.executeUpdate("create table nodevec engine=" + ENGINE +
                    " select A.id node,B.dimension dimension, B.value value from subnodemap A," + dbname + ".nodevec B where A.origin=B.id");
        }
...
create table keymap(select queries.keyword `key`,subnodemap.id node from (dbpedia.keymap join queries on dbpedia.keymap.`key`=queries.keyword)  join  subnodemap on dbpedia.keymap.node=subnodemap.origin);
         */
        stmt.executeUpdate("create table subqueriesmap engine="+ENGINE+"( " +
                ...
                "                   where keyword not in(" +
                "                       select distinct("+dbname+".keymap.`key`) from "+dbname+".keymap join subnodemap on subnodemap.origin="+dbname+".keymap.node" +
                "                   )" +
                "           )" +
                "           group by `query`" +
                "           having count(`keyword`)>=2 and count(`keyword`)<=6" +
                ...
                "   group by `query`" +
                ")");
        stmt.executeUpdate("create table queries engine="+ENGINE+"(" +
        ...
        stmt.executeUpdate("create table keymap engine="+ENGINE+"(" +
    ...
    public static void main(String[]args){
        try{
            GenerateSubDBPedia here=new GenerateSubDBPedia("dbpedia_6m");
            here.connect();
            here.read_graph();
            ...
            GenerateHubLabel generateHubLabel=new GenerateHubLabel(here.subdbname);
            generateHubLabel.solve_sal();
            generateHubLabel.solve_sd();
